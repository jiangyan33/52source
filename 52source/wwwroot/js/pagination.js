function SetPagination(categoryId, currentPage, totalPages) {
    let navStr = "";
    // 处理分页信息
    if (currentPage > 1) {
        navStr += `<span class=first> <a href='javascript:void(0)' onclick="loadCategoryVideos('${categoryId}',1)">首页</a> </span>\n<span class=prev> <a href='javascript:void(0)' rel=prev   onclick="loadCategoryVideos('${categoryId}',${currentPage - 1})">上一页</a> </span>`;
    }
    ...
    if (currentPage <= 6) {
        ...
        for (let i = 1; i <= currentPage; i++) {
            if (i === currentPage) {
                ...
                navStr += `<span class=page> <a href='javascript:void(0)' rel=next onclick="loadCategoryVideos('${categoryId}',${i})">${i}</a> </span>`;
            }
        }
        // 处理当前页后面的数据
        if (totalPages - 6 <= currentPage) {
            ...
            for (let i = 1; i <= length; i++) {
                navStr += `<span class=page> <a href='javascript:void(0)' rel=next    onclick="loadCategoryVideos('${categoryId}',${i + currentPage})">${i + currentPage}</a> </span>`;
            }
        } else {
